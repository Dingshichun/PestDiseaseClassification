...
实现农业病虫害图像分类。具体内容包括图像数据准备、模型训练、模型测试、部署到网页端和部署到 jetson-nano-b01。  
### 数据准备
内容见 [01_DataPreparation](./01_DataPreparation) 文件夹  
...
内容见文件夹 [02_TrainModel](./02_TrainModel)，其中 my_dataset.py 和 utils.py 中是一些数据处理和模型训练的函数。  
...
### 模型测试
内容见 [03_TestModel](./03_TestModel/) 文件夹。  
...
### 部署到网页端
内容见 [04_WebService](./04_WebService/) 文件夹
利用 flask 将模型部署到网页端，在网页端加载本地图像数据并分类，将分类结果进行展示。

### 部署到 jetson-nano-b01
内容见 [05_Deploy2JetsonNano](./05_Deploy2JetsonNano/) 文件夹  
包含 jetson-nano 的基础配置，配置完成之后安装 arm 版本的 pytorch 。  
然后是转换模型，pth——onnx——engine，从 pth 格式的文件转换为 tensorRT 引擎，  
然后进行单张图片的推理和 CSI 摄像头实时推理测试。