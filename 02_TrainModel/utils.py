...
def freeze_model_layers(model, freeze_layers=True, output_layer_names=None):
    """
    冻结模型除输出层外的所有层
    ...
    参数:
        model: 模型
        freeze_layers: 是否冻结
        output_layer_names: 输出层的名称列表，默认包含常见输出层名
    """
    ...
    if output_layer_names is None:
        # 常见模型的输出层名称
        output_layer_names = ['fc', 'classifier', 'head', 'last_linear', 'output']
    ...
    print(f"可训练层 ({len(trainable_layers)} 个):")
